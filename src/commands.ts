...
import { SkillWizardPanel } from './webview/skillWizard';
...
export function registerCommands(
    ...
    localProvider: LocalSkillsProvider,
    githubProvider: GitHubSkillsProvider,
    communityProvider: CommunitySkillsProvider
): void {
    const githubService = new GitHubService();

    // Refresh skills
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.refreshSkills', () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.createSkill', () => {
            SkillWizardPanel.createOrShow(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.installSkill', async (item: any) => {
    ...
    // Open skill file
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.openSkillFile', async (item: any) => {
            if (item?.filePath) {
    ...
    // Open in GitHub
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.openInGitHub', (item: any) => {
            if (item?.skill) {
    ...
    // Delete skill
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.deleteSkill', async (item: any) => {
            if (!item?.skill?.path) return;
    ...
    // Reveal in Explorer
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.revealInExplorer', (item: any) => {
            if (item?.skill?.path) {
    ...
    // Search skills
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.searchSkills', async () => {
    ...
    // Filter skills
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.filterSkills', async () => {
            const categories: SkillCategory[] = ['development', 'design', 'security', 'document', 'testing', 'automation', 'other'];
            ...
            if (selected.value === 'stars') {
                const stars = await vscode.window.showInputBox({
                    ...
                    value: '10',
                    validateInput: (value) => {
                        if (isNaN(parseInt(value))) return 'Enter a number';
                        return null;
                    }
                });
                if (stars) {
                ...
                const period = await vscode.window.showQuickPick([
                    { label: 'Last week', value: '1week' as const },
                    { label: 'Last month', value: '1month' as const },
                    { label: 'Last 3 months', value: '3months' as const },
                    { label: 'Last year', value: '1year' as const }
                ...
                const category = selected.value.replace('category:', '') as SkillCategory;
    ...
    // GitHub Login
    context.subscriptions.push(
        vscode.commands.registerCommand('antigravity.loginGitHub', async () => {
