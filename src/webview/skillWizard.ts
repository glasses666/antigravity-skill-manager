 ...
 */
export class SkillWizardPanel {
    public static currentPanel: SkillWizardPanel | undefined;
    private readonly _panel: vscode.WebviewPanel;
    private readonly _extensionUri: vscode.Uri;
    private _disposables: vscode.Disposable[] = [];
    private _skillsPath: string;

    public static createOrShow(extensionUri: vscode.Uri, skillsPath: string) {
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, skillsPath: string) {
        this._panel = panel;
    ...
    private async _createSkill(data: {
        name: string;
        displayName: string;
        description: string;
        category: string;
        ...
        try {
            ...
            if (data.createData) {
                ...
                fs.writeFileSync(
                    path.join(skillPath, 'data', '.gitkeep'),
                    '',
                    'utf-8'
                );
            }
            ...
            if (data.createExamples) {
                ...
                fs.writeFileSync(
                    path.join(skillPath, 'examples', 'example.md'),
...
`;
    ...
    private _getCategoryKeywords(category: string): string {
